...
TCP协议全称是传输控制协议是一种面向连接的、可靠的、基于字节流的传输层通信协议，由IETF的RFC 793定义。TCP是面向连接的、可靠的流协议。

1. TCP连接
    TCP连接需要进行三次连接
    1. 客户端发送请求连接报文段SYN，之后进入SYN-SEND状态
    2. 服务端收到后，如果同意，则回复SYN与ACK，并进入SYN-RECEIVED状态
    3. 客户端收到服务端回复后，需要再次向服务段发送一个确认报文，表示收到了服务端的回复，发送后，客户端和服务端进入ESTABLISHED状态；此时，连接建立成功；

有人会问为什么要三次而不是两次呢？来个例子：<br>
你在门口的院子玩，你妈妈在家里做事情，突然你妈妈发现要下雨了，想让你收下院子里晾的衣服；然后你妈（客户端）喊了你一句：某某某（你的名字）;你（服务端）听到了（第一次握手）；<br>
你回复了句，谁啊（第二次握手，表示你的消息我接受到了）；<br>
你妈说，我是你妈（第三次握手，表示客户端收到了服务端的回复）；然后你和你妈就知道双方是谁了，可以进行下一步的交流了（数据交互）；

如果没有第三次你妈的回复，那么你不知道是谁，也不知道你的回复有没有人听到，如果这个时候你妈直接喊：收下衣服；你不确定是跟你说话，可能是别人；

2. TCP断开连接

TCP 是全双工的，在断开连接时两端都需要发送FIN和ACK。
